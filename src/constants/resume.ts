...
export const INTRO =
  `프론트엔드 개발자로 출발해, 사용자에게 안정적인 서비스를 제공하는 QA로 확장하고 있습니다. 반복 작업을 자동화해 휴먼에러를 줄이고, 테스트 자동화에 관심을 가지고 학습합니다. 
`
    .split('\n')
    .map((x) => x.trim())
    .filter((x) => x);
...
export const WORK_EXPERIENCE: WorkExperience[] = [
  {
    ...
    project: [
      {
        title: '검수 체크리스트 체계화 및 보완',
        date: '2025.07 - 현재',
        intro:
          '재현 불가능한 버그와 누락된 항목으로 오픈 이후 이슈가 발생한 사례를 계기로, 체크리스트에 버전 관리 체계를 도입',
        description: [
          '체크리스트 버전을 기록하여, <span class="highlight">동일 환경에서 결함 재현 가능성을 확보</span>',
          '정책 변경 시 주요 버전(예: v2.0), 검수 항목 추가 시 소수점 버전(예: v2.1)으로 구분해 버전 관리',
          '파트너사 및 내부 QA 간의 기준 차이를 줄여 커뮤니케이션 효율성을 향상',
        ],
      },
      {
        ...
        date: '2025.07',
        intro:
          '매주 약 80여 개의 미니앱을 검수하는 과정에서 누락된 정보, 중복 등 휴먼 에러가 발생하는 문제를 해결하기 위해 자동화를 도입',
  ...
  },
  {
    ...
    description: [
      '영어 회화 서비스인 링글잉글리시의 프론트엔드 개발자로 3개의 프로젝트에서 신규 기능 개발 및 유지보수를 담당했습니다.',
    ],
    project: [
      {
